...
export class DiscountService {
  ...
  listDiscounts(page:number = 1,search:string){
    ...
    let URL = URL_SERVICE+"/admin/discounts?page="+page+"&search="+search;
  ...
  configDiscounts(){
    this.isLoadingSubject.next(true);
    let headers = new HttpHeaders({'Authorization': 'Bearer '+this.authservice.token});
    let URL = URL_SERVICE+"/admin/cupones/config";
    return this.http.get(URL,{headers: headers}).pipe(
      finalize(() => this.isLoadingSubject.next(false))
    );
  }
  ...
  createDiscounts(data:any){
    this.isLoadingSubject.next(true);
    let headers = new HttpHeaders({'Authorization': 'Bearer '+this.authservice.token});
    let URL = URL_SERVICE+"/admin/discounts";
  ...
  showDiscount(discount_id:string){
    this.isLoadingSubject.next(true);
    let headers = new HttpHeaders({'Authorization': 'Bearer '+this.authservice.token});
    let URL = URL_SERVICE+"/admin/discounts/"+discount_id;
    return this.http.get(URL,{headers: headers}).pipe(
      finalize(() => this.isLoadingSubject.next(false))
    );
  }
